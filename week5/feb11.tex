...
Saying $a = 100*(b_I)^2$ doesn't work because the $b_I$ would not be a constant, since
$b_I$ would grow in size as I grows. You need to show that the Turing Machine stops in a|I| 
steps, so a needs to be a constant. In this case it is not a constant. 


\problem{13. (b)}

What we need to prove here is that with our Turing Machine S that it will take at most
$dn^22^{2n}$ steps. In this case, our n would be equal to our input I. So we are saying
that our Turing Machine S has input I and a string version of our Turing Machine and input
$b_I$ (with some constant c) such that $cI^2(b_I)^2 \leq dn^22^{2n}$. This will work
since the $b_I$ can express $2^n$ numbers with an input length of n digits. Therefore,
the two sides will balance out since $b_I$ squared with be equal to $2^{2n}$.

...
Saying $a = 100*(b_I)^2$ just changes the constant. This constant could be any positive integer,
so while this may work for the constant 100, it may fail with another constant that you have not
tested.

\problem{13. (b)}

 If S can run this simulation in less than $a = 100*(b_I)^2$ steps, then S can run this simulation
in any amount of steps larger than $a = 100*(b_I)^2$ steps. If L is in TIME($n^22^{2n}$), then
there exists a constant such that S runs in $dn^22^{2n}$ due to the Turing Machine S being able
to increase its speed by a constant factor through the packing together tape symbols of the original
tape symbols. Thus, this d may be equal to 100, and  $100(b_I)^22^{2(b_I)}$ will be a larger amount
of steps then $100*(b_I)^2$ steps. This means that S will always be able to run in less than $100(b_I)^22^{2(b_I)}$
steps, so L is in TIME($n^22^{2n}$).

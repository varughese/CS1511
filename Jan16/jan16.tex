...
Assume that $\langle M, w \rangle \in A_{TM}$.  Since $\langle M, w \rangle \in A_{TM}$, M accepts input w, so $L(M') = \Sigma^{*}$. Since $L(M') \neq \emptyset$, $\langle M \rangle \not\in E_{TM}$. Since R is a decider for $E_{TM}$, running input $\langle M' \rangle$ will cause R to reject $\langle M' \rangle$, so S will accept  $\langle M, w \rangle$. 

Assume that $\langle M, w \rangle \not\in A_{TM} $.  Since $\langle M, w \rangle \not\in A_{TM} $, M does not accept input w, so $L(M') = \emptyset $. Since $ L(M') = \emptyset$, $\langle M \rangle \in E_{TM}$. Since R is a decider for $E_{TM}$, running input $\langle M' \rangle$ will cause R to accept $\langle M' \rangle $, so S will reject  $\langle M, w \rangle$. 
...
Assume that $\langle M, w \rangle \in A_{TM}$.  Since $\langle M, w \rangle \in A_{TM}$, M accepts w, so $L(M') = \Sigma^{*}$. Since $L(M') = \Sigma^{*}$, $\langle M \rangle \in E_{TM}$. Since R is a decider for $E_{TM}$, running input $\langle M' \rangle$ will cause R to reject $\langle M' \rangle$, so S will accept  $\langle M, w \rangle$. 
...
\begin{enumerate}[topsep=0pt,itemsep=0pt]
\item Run M on input w
\item If M accepts w, and x = 11110 \underline{accept}. Otherwise \underline{accept}.
\end{enumerate}
\item Run R on input $\langle M' \rangle$

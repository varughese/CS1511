...
Second we prove part 1. We need to show that $C \in EXPSPACE$.

There exists a $TM_C$ that decides $C$.

This $TM_C$ takes an input (I, T, k).

This machine will either halt on accept or reject, or halt and reject if it runs
for more than $2^{2^{|I|^{k}}}$ steps.
...
- $\forall I \ T $ halts on $I$ using space $\leq (c/2)^{{|I|}^k} * 2^{{|I|}^k}$.
...
First we prove part 2. We have a language $C$.
$C = \{ (I, T, k) \mid T \text{ accepts } I \text{ in } \leq  (c/2)^{{|I|}^k} * 2^{{|I|}^k} \text{ space } \}$

Now that we have established  $(c/2)^{{|I|}^k}$ as some constant factor being multiplied to $2^{{|I|}^k}$,
we can dismiss the c since this constant multiplier will not affect it being in EXPSPACE.

The proof will then follow the same path as 17 (a).
